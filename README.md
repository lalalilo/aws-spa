...
- `--viewerRequestFunctionNames`: Add existing CloudFront functions as viewer-request function. Specified functions must already exist or the command will fail. If a lamdba function is already associated, the command will also fail (existing CloudFront functions will be replaced). **Cannot be used together with `--noDefaultRootObject`** — both compete for the single viewer-request function slot; the deploy will fail.
- `--viewerResponseFunctionNames`: Add existing CloudFront functions as viewer-response function. Specified functions must already exist or the command will fail. If a lamdba function is already associated, the command will also fail (existing CloudFront functions will be replaced). Compatible with `--noDefaultRootObject` (uses viewer-response, not viewer-request).
